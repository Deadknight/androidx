...
actual fun createNavGraphBuilder(
    ...
    return PlatformNavGraphBuilder(navHostController.navHostController.navigatorProvider, startDestination, route)
}
...
actual fun PlatformNavGraphBuilder.composable(
    ...
    addDestination(
        ComposeNavigator.Destination(
            navGraphBuilder.provider.getNavigatorWithName(ComposeNavigatorNAME) as ComposeNavigator,
            content
        ).apply {
            this.setRouteInternal(route)
            arguments.forEach { (argumentName, argument) ->
                this.arguments[argumentName] = argument
            }
            deepLinks.forEach { deepLink ->
                this.deepLinks?.add(deepLink)
            }
...
actual fun PlatformNavGraphBuilder.navigation(
    ...
    addDestination(
        PlatformNavGraphBuilder(navGraphBuilder.provider, startDestination, route).apply(builder).build().apply {
            arguments.forEach { (argumentName, argument) ->
                this.arguments[argumentName] = argument
            }
            deepLinks.forEach { deepLink ->
...
actual fun PlatformNavGraphBuilder.dialog(
    ...
    addDestination(
        DialogNavigator.Destination(
            navGraphBuilder.provider.getNavigatorWithName(DialogNavigatorName) as DialogNavigator,
