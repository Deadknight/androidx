...
import androidx.compose.ui.graphics.isHardwareAccelerated
...
internal class RenderNodeLayer(
    ...
    private val renderNode = RenderNodeApiIOS(ownerView).apply { setHasOverlappingRendering(true) }

    override val layerId: ID
        get() = renderNode.uniqueId.toString()
    ...
    override fun drawLayer(canvas: Canvas) {
        ...
        if (androidCanvas.isHardwareAccelerated) {
            ...
            if (renderNode.alpha < 1.0f) {
                ...
                androidCanvas.saveLayerBounds(
                    TIOSKHSkikoRect(
                    left,
                    top,
                    right,
                    bottom),
                    paint.asFrameworkPaint() as TIOSKHTPaintProtocol
                )
            } else {
                canvas.save()
            }
