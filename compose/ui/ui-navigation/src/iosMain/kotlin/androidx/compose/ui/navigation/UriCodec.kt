...
object UriCodec {
    ...
    private fun unexpectedCharacterException(
        ...
        return Exception(
            uri.toString() + "Unexpected character$nameString: $unexpected " + index
        )
    }
    ...
    private fun appendDecoded(
        ...
        val byteBuffer = SimpleByteBuffer(s.length)
        var count = 0
        var i = 0
        while (i < s.length) {
            var c = s[i]
    ...
    private fun flushDecodingByteAccumulator(
        ...
        byteBuffer.flip()
        try {
            builder.append(byteBuffer.getBuffer().decodeToString())
