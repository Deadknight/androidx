 ...
 */
fun TIOSKHSkikoPath.asComposePath(): Path = SkiaBackedPath(this)
 ...
 */
fun Path.asSkiaPath(): TIOSKHSkikoPath =
...
internal class SkiaBackedPath(
    internalPath: TIOSKHSkikoPath = TIOSKHSkikoPath()
) : Path {
    ...
    override var fillType: PathFillType
        get() {
            if (internalPath.fillMode() == TIOSKHSkikoPathFillMode.evenOdd()) {
    ...
    override fun addRect(rect: Rect) {
        internalPath.addRectRect(rect.toSkiaRect(), TIOSKHSkikoPathDirection.counterClockwise(), 0)
    }

    override fun addOval(oval: Rect) {
        internalPath.addOvalOval(oval.toSkiaRect(), TIOSKHSkikoPathDirection.counterClockwise(), 0)
    }
    ...
    override fun addArc(oval: Rect, startAngleDegrees: Float, sweepAngleDegrees: Float) {
        internalPath.addArcOval(oval.toSkiaRect(), startAngleDegrees, sweepAngleDegrees)
    }

    override fun addRoundRect(roundRect: RoundRect) {
        internalPath.addRRectRrect(roundRect.toSkiaRRect(), TIOSKHSkikoPathDirection.counterClockwise(), 0)
    }

    override fun addPath(path: Path, offset: Offset) {
        internalPath.addPathSrc(path.asSkiaPath(), offset.x, offset.y, false)
    }
    ...
    override fun translate(offset: Offset) {
        internalPath.transformMatrix(TIOSKHSkikoMatrix33.companion().makeTranslateDx(offset.x, offset.y), null, true)
    }

    override fun transform(matrix: Matrix) {
        internalPath.transformMatrix(TIOSKHSkikoMatrix33.companion().makeTranslateDx(0f, 0f).apply { setFrom(matrix) }, null, true)
    }
    ...
    override fun op(
        ...
        val path = TIOSKHSkikoPath.companion().makeCombiningOne(
            path1.asSkiaPath(),
            path2.asSkiaPath(),
    ...
    private fun PathOperation.toSkiaOperation() = when (this) {
        PathOperation.Difference -> TIOSKHSkikoPathOp.difference()
        PathOperation.Intersect -> TIOSKHSkikoPathOp.intersect()
        PathOperation.Union -> TIOSKHSkikoPathOp.union_()
        PathOperation.Xor -> TIOSKHSkikoPathOp.xor_()
        PathOperation.ReverseDifference -> TIOSKHSkikoPathOp.reverseDifference()
        else -> TIOSKHSkikoPathOp.xor_()
    }
    ...
    fun TIOSKHSkikoMatrix33.setFrom(matrix: Matrix) {
        require(
            matrix[0, 2] == 0f &&
                matrix[1, 2] == 0f &&
                matrix[2, 2] == 1f &&
