 ...
 * suspends the execution until `this` [Lifecycle] is [LifecycleState.LIFECYCLESTATE_DESTROYED].
 *
 ...
 *             lifecycle.repeatOnLifecycle(LifecycleState.LIFECYCLESTATE_STARTED) {
 ...
 *
 * Warning: [LifecycleState.LIFECYCLESTATE_INITIALIZED] is not allowed in this API. Passing it as a
 * parameter will throw an [IllegalArgumentException].
 *
 * @param state [LifecycleState] in which `block` runs in a new coroutine. That coroutine
...
public suspend fun Lifecycle.repeatOnLifecycle(
    ...
    require(state !== LifecycleState.LIFECYCLESTATE_INITIALIZED) {
        "repeatOnLifecycle cannot start work with the INITIALIZED lifecycle state."
    }

    if (currentState === LifecycleState.LIFECYCLESTATE_DESTROYED) {
        return
    }
    ...
    coroutineScope {
        withContext(kotlinx.coroutines.Dispatchers.Main.immediate) {
            ...
            if (currentState === LifecycleState.LIFECYCLESTATE_DESTROYED) return@withContext

            // Instance of the running repeating coroutine
            var launchedJob: Job? = null

            // Registered observer
            var observer: LifecycleEventObserverProtocol? = null
            try {
                ...
                suspendCancellableCoroutine<Unit> { cont ->
                    ...
                    val startWorkEvent = Lifecycle.upTo(state)
                    val cancelWorkEvent = Lifecycle.downFrom(state)
                    val mutex = Mutex()
                    observer = object : NSObject(), LifecycleEventObserverProtocol {
                        ...
                        override fun onStateChanged(
                            ...
                            if (event == LifecycleEvent.LIFECYCLEEVENT_ON_DESTROY) {
                                cont.resume(kotlin.Unit)
                            }
                        }
                    }
                    this@repeatOnLifecycle.addObserver(observer as LifecycleEventObserverProtocol)
                }
            } finally {
                launchedJob?.cancel()
                observer?.let {
...
/**
 * [LifecycleOwnerProtocol]'s extension function for [Lifecycle.repeatOnLifecycle] to allow an easier
 ...
 *         // restart executing if the lifecycle receives the ON_START event again.
 *         lifecycleScope.launch {
 *             repeatOnLifecycle(LifecycleState.LIFECYCLESTATE_STARTED) {
 *                 uiStateFlow.collect { uiState ->
 *                     updateUi(uiState)
 *                 }
 *             }
 ...
 */
public suspend fun LifecycleOwnerProtocol.repeatOnLifecycle(
    state: LifecycleState,
    block: suspend CoroutineScope.() -> Unit
