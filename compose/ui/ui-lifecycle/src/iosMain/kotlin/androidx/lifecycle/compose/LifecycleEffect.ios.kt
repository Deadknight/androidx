 ...
 *
 * This function should **not** be used to listen for [LifecycleEvent.LIFECYCLEEVENT_ON_DESTROY] because
 * Compose stops recomposing after receiving a [LifecycleEvent.LIFECYCLEEVENT_ON_STOP] and will never be
 * aware of an ON_DESTROY to launch [onEvent].
 *
 * This function should also **not** be used to launch tasks in response to callback
 * events by way of storing callback data as a [LifecycleState] in a [MutableState].
 * Instead, see [currentStateAsState] to obtain a [State<LifecycleState>][State]
 ...
 *
 * @throws IllegalArgumentException if attempting to listen for [LifecycleEvent.LIFECYCLEEVENT_ON_DESTROY]
 */
@Composable
actual fun LifecycleEventEffect(
    ...
    if (event == PlatformLifecycleEvent.ON_DESTROY) {
        throw IllegalArgumentException("LifecycleEventEffect cannot be used to " +
            "listen for LifecycleEvent.LIFECYCLEEVENT_ON_DESTROY, since Compose disposes of the " +
    ...
    DisposableEffect(lifecycleOwner) {
        ...
        lifecycleOwner.lifecycle.lifecycle.addObserver(observer)

        onDispose {
            lifecycleOwner.lifecycle.lifecycle.removeObserver(observer)
        }
    }
}
 ...
 * [LifecycleEvent.LIFECYCLEEVENT_ON_START] or [LifecycleEvent.LIFECYCLEEVENT_ON_STOP] (or any new unique
 ...
 * _both_ [LifecycleEvent.LIFECYCLEEVENT_ON_START] and [LifecycleEvent.LIFECYCLEEVENT_ON_STOP], a [LifecycleEventEffect]
 ...
 * [LifecycleEvent.LIFECYCLEEVENT_ON_START] or [LifecycleEvent.LIFECYCLEEVENT_ON_STOP] event, respectively. If the
 * [LifecycleStartEffect] leaves the composition prior to receiving an [LifecycleEvent.LIFECYCLEEVENT_ON_STOP]
...
actual fun LifecycleStartEffect(
    ...
    val lifecycleStartStopEffectScope = remember(key1) {
        LifecycleStartStopEffectScope(lifecycleOwner.lifecycle.lifecycle)
    }
    LifecycleStartEffectImpl(lifecycleOwner.lifecycleOwner, lifecycleStartStopEffectScope, effects)
}
    ...
    DisposableEffect(lifecycleOwner, scope) {
        ...
        val observer = object : NSObject(), LifecycleEventObserverProtocol {
            ...
            override fun onStateChanged(source: LifecycleOwnerProtocol?, _1: LifecycleEvent) {
                val event = _1
                when (event) {
                    LifecycleEvent.LIFECYCLEEVENT_ON_START -> with(scope) {
                        effectResult = effects()
                    }

                    LifecycleEvent.LIFECYCLEEVENT_ON_STOP -> effectResult?.runStopOrDisposeEffect()
 ...
 * clause when an (ON_STOP)[LifecycleEvent.LIFECYCLEEVENT_ON_STOP] event is received or when cleanup is
 ...
 */
class LifecycleStartStopEffectScope(val lifecycle: Lifecycle) : NSObject(), LifecycleOwner {
     ...
     * observer receives an (ON_STOP)[LifecycleEvent.LIFECYCLEEVENT_ON_STOP] event or must undergo cleanup.
     */
...
actual fun LifecycleResumeEffect(
    ...
    val lifecycleResumePauseEffectScope = remember(key1) {
        LifecycleResumePauseEffectScope(lifecycleOwner.lifecycle.lifecycle)
    }
    LifecycleResumeEffectImpl(lifecycleOwner.lifecycleOwner, lifecycleResumePauseEffectScope, effects)
}
    ...
    DisposableEffect(lifecycleOwner, scope) {
        ...
        val observer = object : NSObject(), LifecycleEventObserverProtocol {
            ...
            override fun onStateChanged(source: LifecycleOwnerProtocol?, _1: LifecycleEvent) {
                val event = _1
                when (event) {
                    LifecycleEvent.LIFECYCLEEVENT_ON_RESUME -> with(scope) {
                        effectResult = effects()
                    }

                    LifecycleEvent.LIFECYCLEEVENT_ON_PAUSE -> effectResult?.runPauseOrOnDisposeEffect()
 ...
 * @param lifecycle The lifecycle being observed by this receiver scope
 */
class LifecycleResumePauseEffectScope(val lifecycle: Lifecycle) : NSObject(), LifecycleOwner {
