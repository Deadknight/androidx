...
internal class PopupLayout(
    ...
    initialPositionProvider: PopupPositionProvider,
    popupId: UUID,
    private val popupLayoutHelper: PopupLayoutHelper = PopupLayoutHelperImpl()
) : AbstractComposeView(composeView.context),
    ViewRootForInspector {

    //Non-final Kotlin subclasses of Objective-C classes are not yet supported
    inner class PopupComposeViewGroup : ViewGroup()
    {
         ...
         */
        override fun setIsFocusable(isFocusable: Boolean) = applyNewFlags(
            0
            /*if (!isFocusable) {
         ...
         */
        override fun onTouchEventEvent(event: MotionEvent): Boolean {
            if (!properties.dismissOnClickOutside) {
                return super.onTouchEventEvent(event)
            }
            ...
            if ((event.action == ACTION_DOWN) &&
                ...
                return true
            } else if (event?.action == ACTION_OUTSIDE) {
                onDismissRequest?.invoke()
                return true
            }
    ...
    init {
        self = PopupComposeViewGroup()
        self.kParentType = this
        self.init(composeView.context, null)
        self.id = "android.R.id.content"
        self.setViewTreeLifecycleOwner(composeView.findViewTreeLifecycleOwner())
        self.setViewTreeViewModelStoreOwner(composeView.findViewTreeViewModelStoreOwner())
        self.setViewTreeSavedStateRegistryOwner(composeView.findViewTreeSavedStateRegistryOwner())
        // Set unique id for AbstractComposeView. This allows state restoration for the state
        // defined inside the Popup via rememberSaveable()
        self.setTag("R.id.compose_view_saveable_id_tag", "Popup:$popupId" as NSString)
    ...
    fun show() {
        self.windowManager.addView(self, params)
    }
    ...
    override fun internalOnMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        if (properties.usePlatformDefaultWidth) {
            ...
            val displayWidthMeasureSpec = self.makeMeasureSpecMeasureSpec(displayWidth, MeasureSpec.AT_MOST)
            val displayHeightMeasureSpec = self.makeMeasureSpecMeasureSpec(displayHeight, MeasureSpec.AT_MOST)
    ...
    override fun internalOnLayout(changed: Boolean, left: Int, top: Int, right: Int, bottom: Int) {
        ...
        if (!properties.usePlatformDefaultWidth) {
            ...
            popupLayoutHelper.updateViewLayout(self.windowManager, self, params)
        }
    }
    ...
    fun updateParameters(
        ...
        self.setIsFocusable(properties.focusable)
    ...
    fun updatePosition() {
        ...
        if (properties.excludeFromSystemGesture) {
            ...
            // edge of the screen.
            popupLayoutHelper.setGestureExclusionRects(self, windowSize.width, windowSize.height)
        }
     ...
     */
    fun dismiss() {
        self.setViewTreeLifecycleOwner(null)
        self.windowManager.removeViewImmediate(self)
    }
    ...
    private fun superSetLayoutDirection(layoutDirection: LayoutDirection) {
        /*val direction = when (layoutDirection) {
            LayoutDirection.Ltr -> androidx.compose.ui.platform.LayoutDirection.LTR
            LayoutDirection.Rtl -> androidx.compose.ui.platform.LayoutDirection.RTL
        }
    ...
    private companion object {
        private val onCommitAffectingPopupPosition = { popupLayout: PopupLayout ->
            if (popupLayout.self.isAttachedToWindow) {
                popupLayout.updatePosition()
            }
        }
    }
...
fun isPopupLayout(view: View, testTag: String? = null): Boolean =
    view.kParentType is PopupLayout && (testTag == null || testTag == (view.kParentType as PopupLayout).testTag)
