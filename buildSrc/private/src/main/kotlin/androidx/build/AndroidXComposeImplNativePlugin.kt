...
class AndroidXComposeImplNativePlugin : Plugin<Project> {
    override fun apply(project: Project) {
        val extension =
            project.extensions.create<AndroidXComposeNativeExtension>("androidxComposeNative", project)
...
const val COMPILER_NATIVE_PLUGIN_CONFIGURATION = "kotlinNativePlugin"

private fun configureComposeCompilerPlugin(project: Project, extension: AndroidXComposeNativeExtension) {
    project.afterEvaluate {
        ...
        project.dependencies.add(
            ...
            if (ProjectLayoutType.isPlayground(project)) {
                AndroidXPlaygroundRootImplPlugin.projectOrArtifact(
                    project.rootProject,
                    ":compose:compiler:compiler-hosted"
                )
            } else {
                project.rootProject.resolveProject(":compose:compiler:compiler-hosted")
            }
        )
        ...
        project.tasks.withType(KotlinNativeCompile::class.java).configureEach { compile ->
            println(compile.shortModuleName)
            println(compile.kotlinOptions.freeCompilerArgs)
            println(compile.compilerOptions.freeCompilerArgs)
            /*compile.inputs
                .files({ kotlinNativePlugin })
            ...
            compile.doFirst {
                /*compile.kotlinOptions {
                    freeCompilerArgs += "-Xplugin=${kotlinNativePlugin.first()}"
                }*/
                //compile.compilerOptions.freeCompilerArgs.add("-Xplugin=${kotlinNativePlugin.first()}")
                //compile.kotlinOptions.freeCompilerArgs += "-Xplugin=${kotlinNativePlugin.first()}"
