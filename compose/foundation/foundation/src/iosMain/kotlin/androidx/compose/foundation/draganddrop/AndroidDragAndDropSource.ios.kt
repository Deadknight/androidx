...
private class CacheDrawScopeDragShadowCallback {
    private var cachedPicture: TIOSKHSkikoBitmap? = null
    ...
    fun drawDragShadow(drawScope: DrawScope) = with(drawScope) {
        when (val picture = cachedPicture) {
            null -> throw IllegalArgumentException(
                ...
                canvas.nativeCanvas.drawImage(TIOSKHSkikoImage.companion().makeFromBitmapBitmap(cachedPicture!!), 0f, 0f)
            }
        }
    }
    ...
    fun cachePicture(scope: CacheDrawScope): DrawResult = with(scope) {
        ...
        val picture = ImageInfo.makeN32Premul(width, height)
