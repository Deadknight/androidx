...
internal class CompositionContextWrapper(val compositionContext: CompositionContext) : NSObject()

var View.compositionContext: CompositionContext?
    get() = (getTag("R.id.androidx_compose_ui_view_composition_context") as? CompositionContextWrapper)?.compositionContext
    set(value) {
        setTag("R.id.androidx_compose_ui_view_composition_context", if(value != null) CompositionContextWrapper(value) else null)
    }
...
private val animationScaleLock = SyncLock()
...
private fun getAnimationScaleFlowFor(applicationContext: Context): StateFlow<Float> {
    return synchronized(animationScaleLock) {
...
object WindowRecomposerPolicy {
    ...
    internal fun createAndInstallWindowRecomposer(rootView: View): Recomposer {
        ...
        rootView.addOnAttachStateChangeListener(
            object : OnAttachStateChangeListenerProtocol, NSObject() {
...
fun View.createLifecycleAwareWindowRecomposer(
    ...
    viewTreeLifecycle.addObserver(
        object : LifecycleEventObserverProtocol, NSObject() {
            val key = NSUUID.randomUUID().UUIDString
            ...
            override fun onStateChanged(source: LifecycleOwnerProtocol?, _1: LifecycleEvent) {
                ...
                val self = this
                when (event) {
                    LifecycleEvent.LIFECYCLEEVENT_ON_CREATE -> {
                    ...
                    LifecycleEvent.LIFECYCLEEVENT_ON_START -> {
                    ...
                    LifecycleEvent.LIFECYCLEEVENT_ON_STOP -> {
                    ...
                    LifecycleEvent.LIFECYCLEEVENT_ON_DESTROY -> {
                        recomposer.cancel()
                    }
                    LifecycleEvent.LIFECYCLEEVENT_ON_PAUSE -> {
                        // Nothing
                    }
                    LifecycleEvent.LIFECYCLEEVENT_ON_RESUME -> {
                        // Nothing
                    }
                    LifecycleEvent.LIFECYCLEEVENT_ON_ANY -> {
                        // Nothing
                    }
