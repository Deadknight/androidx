...
import ACTION_CANCEL
import ACTION_DOWN
import ACTION_HOVER_ENTER
import ACTION_HOVER_EXIT
import ACTION_HOVER_MOVE
import ACTION_MOVE
import ACTION_POINTER_DOWN
import ACTION_POINTER_UP
import ACTION_SCROLL
import ACTION_UP
...
import PointerProperties
import RequiresApi
import SavedStateRegistryOwner
import SuppressLint
import SystemClock
import TOOL_TYPE_MOUSE
import View
import ViewGroup
import actionIndex
import actionMasked
import addView
import android.graphics.IDENTITY
...
import android.graphics.postTranslate
...
import clearFocus
import clipRect
import cocoapods.ToppingCompose.ACTION_DRAG_ENDED
import cocoapods.ToppingCompose.ACTION_DRAG_ENTERED
import cocoapods.ToppingCompose.ACTION_DRAG_EXITED
import cocoapods.ToppingCompose.ACTION_DRAG_LOCATION
import cocoapods.ToppingCompose.ACTION_DRAG_STARTED
import cocoapods.ToppingCompose.ACTION_DROP
import cocoapods.ToppingCompose.AT_MOST
import cocoapods.ToppingCompose.Configuration
import cocoapods.ToppingCompose.DefaultLifecycleObserverProtocol
...
import cocoapods.ToppingCompose.LuaThread
import cocoapods.ToppingCompose.MeasureSpec
import cocoapods.ToppingCompose.MetalView
import cocoapods.ToppingCompose.MetalViewDelegateProtocol
import cocoapods.ToppingCompose.OnDragListenerProtocol
import cocoapods.ToppingCompose.TIOSKHRect
import cocoapods.ToppingCompose.TIOSKHSkiaCanvas
import cocoapods.ToppingCompose.TIOSKHSkikoCanvas
import cocoapods.ToppingCompose.TIOSKHSkikoMatrix33
import cocoapods.ToppingCompose.TIOSKHTCanvasProtocol
import cocoapods.ToppingCompose.UNSPECIFIED
import cocoapods.ToppingCompose.ViewTreeObserverOnGlobalLayoutListenerProtocol
import cocoapods.ToppingCompose.ViewTreeObserverOnScrollChangedListenerProtocol
import cocoapods.ToppingCompose.ViewTreeObserverOnTouchModeChangeListenerProtocol
...
import flags
import getChildAt
import getLocationOnScreen
import getLocationOnWindow
import getPointerCoords
import getPointerId
import getPointerProperties
import getRawX
import getRawY
import getToolType
import getX
...
import isHardwareAccelerated
import isInTouchMode
...
import rawY
import recycle
import removeCallbacks
import removeViewInLayout
import requestFocusFromTouch
import resources
import scrollX
import scrollY
...
internal class AndroidComposeView(context: Context, coroutineContext: CoroutineContext) :
    Owner, ViewRootForTest, PositionCalculator {

    val self: LifecycleViewGroup
    val rendererDelegate: MetalViewDelegateProtocol

    inner class LifecycleViewGroup : ViewGroup(), DefaultLifecycleObserverProtocol {

        @OptIn(ExperimentalForeignApi::class)
        override fun createComponent(): UIView? {
            val view = MetalView(CGRectMake(dX.toDouble(), dY.toDouble(), dWidth.toDouble(), dHeight.toDouble()), MTLCreateSystemDefaultDevice(), self)
            //view.setFrame(CGRectMake(dX.toDouble(), dY.toDouble(), dWidth.toDouble(), dHeight.toDouble()))
        ...
        override fun onCreate(owner: LifecycleOwnerProtocol?) {}
        override fun onDestroy(owner: LifecycleOwnerProtocol?) {}
        override fun onPause(owner: LifecycleOwnerProtocol?) {}
        override fun onStart(owner: LifecycleOwnerProtocol?) {}
        override fun onStop(owner: LifecycleOwnerProtocol?) {}
        ...
        override fun onFocusChanged(gainFocus: Boolean, _1: Int, _2: TIOSKHRect?) {
            val direction = _1
            val previouslyFocusedRect = _2
            super.onFocusChanged(gainFocus, direction, previouslyFocusedRect)
            Log.d(FocusTag, "Owner FocusChanged($gainFocus)")
            focusOwner.focusTransactionManager.withExistingTransaction(
                onCancelled = { if (gainFocus) self.clearFocus() else requestFocus() }
            ) {
        ...
        override fun onMeasure(widthMeasureSpec: Int, _1: Int) {
            ...
            trace("AndroidOwner:onMeasure") {
                ...
                var (minWidth, maxWidth) = convertMeasureSpec(widthMeasureSpec)
                var (minHeight, maxHeight) = convertMeasureSpec(heightMeasureSpec)
                ...
                self.setMeasuredDimension(root.width, root.height)

                if (_androidViewsHandler != null) {
                    androidViewsHandler.measure(
                        MeasureSpec.makeMeasureSpec(root.width, EXACTLY.toInt()),
                        MeasureSpec.makeMeasureSpec(root.height, EXACTLY.toInt())
                    )
                }
            }
        }

        override fun layout(l: int32_t, _1: int32_t, _2: int32_t, _3: int32_t) {
            val t = _1
            val r = _2
            val b = _3
        ...
        override fun onDrawCanvas(canvas: TIOSKHTCanvasProtocol) {
        ...
        override fun onAttachedToWindow() {
            ...
            val lifecycleOwner = self.findViewTreeLifecycleOwner()
            val savedStateRegistryOwner = self.findViewTreeSavedStateRegistryOwner()
            ...
            if (resetViewTreeOwner) {
                ...
                oldViewTreeOwners?.lifecycleOwner?.lifecycle?.removeObserver(self)
                lifecycleOwner.lifecycle?.addObserver(self)
            ...
            _inputModeManager.inputMode = if (self.isInTouchMode) Touch else Keyboard
            ...
            self.viewTreeObserver!!.addOnGlobalLayoutListener(globalLayoutListener)
            self.viewTreeObserver!!.addOnScrollChangedListener(scrollChangedListener)
            self.viewTreeObserver!!.addOnTouchModeChangeListener(touchModeChangeListener)
        }

        override fun onDetachedFromWindow() {
            super.onDetachedFromWindow()
            snapshotObserver.stopObserving()
            viewTreeOwners?.lifecycleOwner?.lifecycle?.removeObserver(self)
            ...
            self.viewTreeObserver!!.removeOnGlobalLayoutListener(globalLayoutListener)
            self.viewTreeObserver!!.removeOnScrollChangedListener(scrollChangedListener)
            self.viewTreeObserver!!.removeOnTouchModeChangeListener(touchModeChangeListener)
        }
        ...
        override fun dispatchGenericMotionEvent(event: MotionEvent?) = when (event!!.actionMasked) {
            ACTION_SCROLL -> when {
                ...
                isBadMotionEvent(event) || !self.isAttachedToWindow ->
        ...
        override fun dispatchTouchEvent(motionEvent: MotionEvent?): Boolean {
            ...
            if (hoverExitReceived) {
                ...
                // pointer
                self.removeCallbacks(sendHoverExitEvent)
            ...
            if (isBadMotionEvent(motionEvent) || !self.isAttachedToWindow) {
            ...
            if (processResult.anyMovementConsumed) {
                if(self.parent is LGViewGroup)
                    (self.parent as LGViewGroup).requestDisallowInterceptTouchEvent(true)
            }
        ...
        override fun onConfigurationChanged(newConfig: Configuration?) {
            ...
            if (newConfig.fontWeightAdjustmentCompat != currentFontWeightAdjustment) {
                ...
                fontFamilyResolver = createFontFamilyResolver(self.context)
            }
        ...
        override fun onRtlPropertiesChanged(layoutDirection: Int) {
            ...
            if (superclassInitComplete) {
                layoutDirectionFromInt(layoutDirection).let {
                    (kParentType as AndroidComposeView).layoutDirection = it
    ...
    init {
        self = LifecycleViewGroup()
        self.setKParentType(this)
        self.init(context, null)
        self.forceOverrideDrawRect = true
        self.setForceOverrideDrawRectBlock {
            (self._view as MetalView).redrawer?.needRedraw()
            false
        }
        self.lc = context
        self._view!!.backgroundColor = UIColor.clearColor
        self._view!!.opaque = false
        rendererDelegate = object : NSObject(), MetalViewDelegateProtocol {
            override fun onRender(canvas: TIOSKHSkikoCanvas?, _1: Int, _2: Int, _3: Long) {
                canvas?.let {
                    val canvasWrap = TIOSKHSkiaCanvas(canvas)
                    self.onDrawCanvas(canvasWrap)
                    self.dispatchDrawCanvas(canvasWrap)
                }
            }

            override fun tick(link: CADisplayLink?) {
            }
        }
        (self._view as MetalView).setMetalViewDelegate(rendererDelegate)
    }

    override fun requestFocus(): Boolean {
        return self._view?.becomeFirstResponder() ?: false
    }
    ...
    override val view: View get() = self
    ...
    private val _autofill = if (autofillSupported()) AndroidAutofill(self, autofillTree) else null
    ...
    override val accessibilityManager: AccessibilityManager
        get() = object : AccessibilityManager {
    ...
    internal val androidViewsHandler: AndroidViewsHandler
        get() {
            if (_androidViewsHandler == null) {
                _androidViewsHandler = AndroidViewsHandler(self.context)
                self.addView(_androidViewsHandler)
            }
    ...
    private val globalLayoutListener = object : NSObject(), ViewTreeObserverOnGlobalLayoutListenerProtocol {
    ...
    private val scrollChangedListener = object : NSObject(), ViewTreeObserverOnScrollChangedListenerProtocol {
    ...
    private val touchModeChangeListener = object : NSObject(), ViewTreeObserverOnTouchModeChangeListenerProtocol {
        override fun onTouchModeChanged(touchMode: Boolean) {
    ...
    private val textInputSessionMutex = SessionMutex<IOSPlatformTextInputSession>()

    override suspend fun textInputSession(
        session: suspend PlatformTextInputSessionScope.() -> Nothing
    ): Nothing = textInputSessionMutex.withSessionCancellingPrevious(
        sessionInitializer = {
            IOSPlatformTextInputSession(
                view = self,
                coroutineScope = it
            )
        },
    ...
    private val _inputModeManager = InputModeManagerImpl(
        initialInputMode = if (self.isInTouchMode) Touch else Keyboard,
        onRequestInputModeChange = {
            when (it) {
                ...
                Keyboard -> if (self.isInTouchMode) self.requestFocusFromTouch() else true
     ...
     */
    override val textToolbar: TextToolbar = AndroidTextToolbar(self)
        ...
        if (lastEvent != null) {
            when (lastEvent.actionMasked) {
                ...
                ACTION_HOVER_ENTER, ACTION_HOVER_MOVE -> {
                    ...
                    self.post(resendMotionEventRunnable)
                }
            }
        }
    }
    ...
    override fun drag(dragAndDropInfo: DragAndDropInfo): Boolean {
        val density = with(self.context.resources) {
            Density(
                density = displayMetrics.density,
                fontScale = configuration.fontScale
            )
        }
        self.startDragAndDrop(dragAndDropInfo.transfer.clipData.toString())
        return true
    ...
    private fun clearChildInvalidObservations(viewGroup: ViewGroup) {
        for (i in 0 until viewGroup.childCount) {
            ...
            if (child.kParentType is AndroidComposeView) {
                (child.kParentType as AndroidComposeView).onEndApplyChanges()
    ...
    fun addAndroidView(view: AndroidViewHolder, layoutNode: LayoutNode) {
        ...
        androidViewsHandler.addView(view.self)
    ...
    fun removeAndroidView(view: AndroidViewHolder) {
        registerOnEndApplyChangesListener {
            androidViewsHandler.removeViewInLayout(view.self)
    ...
    private fun scheduleMeasureAndLayout(nodeToRemeasure: LayoutNode? = null) {
        if (!self.isLayoutRequested && self.isAttachedToWindow) {
    ...
    private fun convertMeasureSpec(measureSpec: Int): ULong {
        ...
        return when (mode.toLong()) {
            EXACTLY -> pack(size, size)
            UNSPECIFIED -> pack(0, Constraints.Infinity)
            AT_MOST -> pack(0, size)
    ...
    private fun updatePositionCacheAndDispatch() {
        ...
        self.getLocationOnScreen(tmpPositionArray)
    ...
    override fun createLayer(
        ...
        if (viewLayersContainer == null) {
            if (!ViewLayer.hasRetrievedMethod) {
                ...
                val viewIn = View()
                viewIn.lc = self.context
                ViewLayer.updateDisplayList(viewIn)
            }
            viewLayersContainer = if (ViewLayer.shouldUseDispatchDraw) {
                DrawChildContainer(self.context)
            } else {
                ViewLayerContainer(self.context)
            }
            self.addView(viewLayersContainer!!.self)
        }
    ...
    private fun handleMotionEvent(motionEvent: MotionEvent): ProcessResult {
        self.removeCallbacks(resendMotionEventRunnable)
        try {
...
//            ACTION_CANCEL,
    ...
    private fun sendSimulatedEvent(
        ...
        val pointerProperties = Array(pointerCount) { PointerProperties() }
        val pointerCoords = Array(pointerCount) { PointerCoords() }
        ...
        val event = MotionEvent.obtain(
            ...
            /* yPrecision */ motionEvent.yPresicion,
    ...
    private fun isInBounds(motionEvent: MotionEvent): Boolean {
        val x = motionEvent.x
        val y = motionEvent.y
        return (x in 0f..self.width.toFloat() && y in 0f..self.height.toFloat())
    }
    ...
    private fun recalculateWindowPosition() {
        if (!forceUseMatrixCache) {
            ...
            if (animationTime != lastMatrixRecalculationAnimationTime) {
                ...
                view.getLocationOnWindow(tmpPositionArray)
    ...
    private fun recalculateWindowViewTransforms() {
        matrixToWindow.calculateMatrixToWindow(self, viewToWindowMatrix)
   ...
   /*public override fun dispatchHoverEvent(event: MotionEvent): Boolean {
    ...
    override val isLifecycleInResumedState: Boolean
        get() = viewTreeOwners?.lifecycleOwner
            ?.lifecycle?.currentState == LifecycleState.LIFECYCLESTATE_RESUMED

    //TODO(ios)
    //override fun shouldDelayChildPressedState(): Boolean = false

    private val FocusTag = "Compose Focus"
    private val MaximumLayerCacheSize = 10
    /*companion object {
...
internal val Configuration.localeLayoutDirection: LayoutDirection
    ...
    get() = layoutDirectionFromInt(getLayoutDirection())
...
/**
 * Like [android.graphics.Matrix], for a Compose [Matrix]
 */
...
private class CalculateMatrixToWindowApi29 : CalculateMatrixToWindow {
    private var tmpMatrix = android.graphics.Matrix.IDENTITY
    private val tmpPosition = IntArray(2)

    @DoNotInline
    override fun calculateMatrixToWindow(view: View, matrix: Matrix) {
        tmpMatrix = android.graphics.Matrix.IDENTITY
        ...
        root.getLocationOnWindow(tmpPosition)
...
private class CalculateMatrixToWindowApi21(private val tmpMatrix: Matrix) :
    ...
    private fun transformMatrixToWindow(view: View, matrix: Matrix) {
        ...
        if (parentView is View) {
            ...
            val pos = tmpLocation
            view.getLocationOnWindow(pos)
            matrix.preTranslate(-view.scrollX.toFloat(), -view.scrollY.toFloat())
            matrix.preTranslate(pos[0].toFloat(), pos[1].toFloat())
        }

        val viewMatrix = view.matrix
        if (viewMatrix != TIOSKHSkikoMatrix33.IDENTITY) {
            matrix.preConcat(viewMatrix)
        }
    }

    /**
     * Like [android.graphics.Matrix], for a Compose [Matrix] that accepts an [other]
 ...
 */
private class DragAndDropModifierOnDragListener : NSObject(), OnDragListenerProtocol {
    ...
    override fun onDrag(
        ...
        type = when (_1.action.toLong()) {
            ACTION_DRAG_STARTED -> DragAndDropEventType.Started

            ACTION_DRAG_ENTERED -> DragAndDropEventType.Entered

            ACTION_DRAG_LOCATION -> DragAndDropEventType.Moved

            ACTION_DRAG_EXITED -> DragAndDropEventType.Exited

            ACTION_DROP -> DragAndDropEventType.Dropped

            ACTION_DRAG_ENDED -> DragAndDropEventType.Ended.also {
...
@OptIn(ExperimentalForeignApi::class)
val LGView.matrix: TIOSKHSkikoMatrix33
    get() {
        _view!!.layer.transform.useContents {
            val matrix = this
            val matrixRet = TIOSKHSkikoMatrix33.companion().IDENTITY()
            val mat = matrixRet.mat()
            val iden = TIOSKHSkikoMatrix33.companion().IDENTITY()
            for(i in 0 until mat.size())
            {
                mat.setIndex(i, iden.mat().getIndex(i))
            }
            mat[kMScaleX] = matrix.m11.toFloat()
            mat[kMSkewX] = matrix.m21.toFloat()
            mat[kMTransX] = matrix.m41.toFloat()
            mat[kMPersp0] = matrix.m14.toFloat()
            mat[kMSkewY] = matrix.m12.toFloat()
            mat[kMScaleY] = matrix.m22.toFloat()
            mat[kMTransY] = matrix.m42.toFloat()
            mat[kMPersp1] = matrix.m24.toFloat()
            return matrixRet
        }
    }

fun LGView.transformMatrixToGlobal(matrix: android.graphics.Matrix) {
    /*val vparent = parent as LGViewGroup
    if(vparent is View) {
        vparent.transformMatrixToGlobal(matrix)
        matrix.makeTranslate(-vparent.mScrollX, -vparent.mScrollY)
    }

    matrix.makeTranslate(getMLeft(), getMTop())*/

    //TODO hasidentifymatrix?
}