...
var NavArgument.type: NavType<Any>
    get() {
        return typeObject as NavType<Any>
    }
    set(value) {
        typeObject = value
    }

private val Uri.query: String?
    get() {
        return getQuery()
    }

class Matcher(val matchResult: MatchResult?) {
    var currentResult : MatchResult? = null
    ...
    fun group(group: Int): String {
        return currentResult!!.groupValues[group]
    }

    fun start(): Int {
        return currentResult!!.range.first
    }

    fun end(): Int {
        return currentResult!!.range.last
    }
...
class Pattern(val regex: Regex) {

    fun matcher(str: String): Matcher {
        return Matcher(regex.matchEntire(str))
    }

    companion object {

        val DOTALL = RegexOption.DOT_MATCHES_ALL
        ...
        fun compile(pattern: String, option: RegexOption) : Pattern {
            return Pattern(Regex(pattern, option))
        }

        fun quote(regex: String): String {
...
public class NavDeepLink internal constructor(
    ...
    internal fun calculateMatchingPathSegments(requestedLink: Uri?): Int {
        ...
        val uriPathSegments = Uri.parse(uriPattern).getPathSegments()
    ...
    private fun parseQuery(): MutableMap<String, ParamQuery> {
        ...
        if(uriPattern == null) return paramArgMap
    ...
    private fun parseFragment(): Pair<MutableList<String>, String>? {
        ...
        val fragment = Uri.parse(uriPattern).getFragment()
...
internal fun Map<String, NavArgument?>.missingRequiredArguments(
    ...
    val requiredArgumentKeys = filterValues {
        if (it != null) {
            !it.mIsNullable && it.mDefaultValue == null
        } else false
    }.keys
