...
internal fun AndroidVectorParser.parseCurrentVectorNode(
    ...
    when (xmlParser.eventType) {
        ...
        XmlPullParser.END_TAG -> {
            if (SHAPE_GROUP == xmlParser.name.toString()) {
    ...
    val tintColor = if (
        ...
        if (value.type == TypedValue.TYPE_ATTRIBUTE) {
            ...
            if (tintColorStateList != null) {
                Color(TIOSKHTColorCompanion.shared().toIntInternalColor(tintColorStateList.color!!))
            } else {
                Color.Unspecified
            }
        }
...
@SuppressWarnings("RestrictedApi")
private fun obtainBrushFromComplexColor(complexColor: LuaColor): Brush? =
    //TODO:IOS
...
internal data class AndroidVectorParser(
    ...
    private val changingConfigurations: Int = 0
    ...
    fun obtainAttributes(
        ...
        return TypedArray(res.context, set)
    }
     ...
     */
    fun getFloat(typedArray: TypedArray, index: Pair<String?, String>, defaultValue: Float): Float {
        with(typedArray) {
            val result = getFloat(
                index.first,
                index.second,
     ...
     */
    fun getInt(typedArray: TypedArray, index: Pair<String?, String>, defaultValue: Int): Int {
        with(typedArray) {
            val result = getInt(index.first, index.second, defaultValue)
            updateConfig(changingConfigurations)
            return result
        }
    }
     ...
     */
    fun getString(typedArray: TypedArray, index: Pair<String?, String>): String? {
        with(typedArray) {
            val result = getString(index.first, index.second)
            updateConfig(changingConfigurations)
            return result
        }
    }
     ...
     */
    fun getDimension(typedArray: TypedArray, index: Pair<String?, String>, defValue: Float): Float {
        with(typedArray) {
            val result = getDimension(index.second, defValue)
            updateConfig(changingConfigurations)
            return result
        }
    }
