...
internal class AndroidStringDelegate : PlatformStringDelegate {
    override fun toUpperCase(string: String, locale: PlatformLocale): String =
        (string as NSString).uppercaseStringWithLocale((locale as AndroidLocale).javaLocale)

    override fun toLowerCase(string: String, locale: PlatformLocale): String =
        (string as NSString).lowercaseStringWithLocale((locale as AndroidLocale).javaLocale)

    override fun capitalize(string: String, locale: PlatformLocale): String =
        string.replaceFirstChar {
            if (it.isLowerCase())
            {
                val str : NSString = NSString.stringWithString(it.toString()) as NSString
                str.uppercaseStringWithLocale((locale as AndroidLocale).javaLocale)
                (str as String)[0].toString()
            }
            else it.toString()
        }
    ...
    override fun decapitalize(string: String, locale: PlatformLocale): String =
        string.replaceFirstChar {
            val str : NSString = NSString.stringWithString(it.toString()) as NSString
            str.lowercaseStringWithLocale((locale as AndroidLocale).javaLocale)
            (str as String)[0].toString()
        }
}
