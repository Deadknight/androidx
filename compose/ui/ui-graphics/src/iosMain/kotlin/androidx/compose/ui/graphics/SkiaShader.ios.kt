...
internal actual fun ActualLinearGradientShader(
    ...
    return Shader.companion().makeLinearGradientX0(
        from.x, from.y, to.x, to.y, colors.toIntArray().toTIOSKotlinIntArray(), colorStops?.toFloatArray()?.toTIOSKotlinFloatArray(),
...
internal actual fun ActualRadialGradientShader(
    ...
    validateColorStops(colors, colorStops)
    return Shader.companion().makeRadialGradientX(
        center.x,
        center.y,
        radius,
        colors.toIntArray().toTIOSKotlinIntArray(),
        colorStops?.toFloatArray()?.toTIOSKotlinFloatArray(),
        GradientStyle(tileMode.toSkiaTileMode(), true, identityMatrix33())
    )
}
...
internal actual fun ActualSweepGradientShader(
    ...
    validateColorStops(colors, colorStops)
    return Shader.companion().makeSweepGradientX(
        center.x,
        center.y,
        colors.toIntArray().toTIOSKotlinIntArray(),
        colorStops?.toFloatArray()?.toTIOSKotlinFloatArray()
...
internal actual fun ActualImageShader(
    ...
    return image.asSkiaBitmap().makeShaderTmx(
